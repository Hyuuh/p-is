...
export class TestHandler {
	...
	generateNumberTest(
		...
		for (let sectionIndex = 0; sectionIndex < TOTAL_SECTIONS; sectionIndex++) {
			for (let incision = 0; incision < TOTAL_INCISIONS; incision++) {
				const numbers = randomizeArray([...NUMBERS_ARRAY], SECTION_ENUM[sectionIndex]) as number[];
				test.push({
	...
	generateNumberWithLetterTest(SECTION_ENUM: number[],
		TOTAL_INCISIONS: number) {
		const TOTAL_SECTIONS = SECTION_ENUM.length;
		const test: TypeTest[] = []
		for (let sectionIndex = 0; sectionIndex < TOTAL_SECTIONS; sectionIndex++) {
			for (let incision = 0; incision < TOTAL_INCISIONS; incision++) {
				const numbersArray = shuffle(randomizeArray([...NUMBERS_ARRAY], NUMBERS_ARRAY.length)) as number[];
				const stringArray = shuffle(randomizeArray([...LETTERS_ARRAY], LETTERS_ARRAY.length)) as string[];
				const result = []
	...
	setup() {
		...
		const thirdTest = this.generateNumberWithLetterTest([2, 2, 3, 3, 3, 4, 5, 6, 7, 8], 3)
		this.tests.push({
			id: 1,
			errors: 0,
			sections: 8,
